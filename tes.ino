...
//MYSQL
char user[] = "root";          // Username database MySQL
char password[] = "";          // Password database MySQL
char host[] = "192.168.1.102";  // Alamat IP server MySQL
char db[] = "server";    // Nama database MySQL
...
//kirim database
void sendToDatabase(float tdsValue, float celsius) {
  if (conn.connect(host, 3306, user, password)) {
    Serial.println("Connected to MySQL server");
    MySQL_Cursor *cur_mem = new MySQL_Cursor(&conn);

    // Query INSERT ke tabel hidroponik
    cur_mem->execute("INSERT INTO hidroponik (tds, temp_hidro) VALUES (%f, %f)", tdsValue, celsius);
...
void sendiot() {
 ...
 sendToDatabase(tdsValue, celsius);
  } else {
    Serial.println("Problem updating channel. HTTP error code " + String(x));
  }
